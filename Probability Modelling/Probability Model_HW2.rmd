...
R program that takes an uniform distribution and generates random variables from an exponential distribution.
...
y<-dexp(x,rate = 3)
dat<-data.frame(x,y)
g<-ggplot(data=dat,aes(x=x,y=y))+ggtitle("X: Uniform Distribution, Y: Exponential Distribution")
g<-g+theme(plot.title = element_text(hjust = 0.5))+geom_line()
g
...
g<-ggplot(data=dat,aes(x=x,y=y))+geom_line()+ggtitle("Normal Distribution")
g<-g+theme(plot.title = element_text(hjust = 0.5))
g<-g+xlab("X")+ylab("pmf(x)")+scale_x_continuous(breaks=round(c(-12,-9,-6,xx,0,3,6,round(xx,2),12),2))+geom_vline(xintercept = xx)
